...
async function getApiKey(): Promise<string | undefined> {
    ...
    let apiKey = vscode.workspace.getConfiguration('codecat').get<string>('anthropicApiKey');
 ...
 */
export async function runCustomCommand(code: string, command: CustomCommand): Promise<string | null> {
    try {
        // Get API key
 ...
 */
export async function registerCustomCommands(context: vscode.ExtensionContext): Promise<vscode.Disposable[]> {
    ...
    for (const command of commands) {
        ...
        const disposable = vscode.commands.registerCommand(command.id, async (uri: vscode.Uri, selectedUris: vscode.Uri[]) => {
 ...
 */
export async function getCodeFromFiles(files: vscode.Uri[]): Promise<string> {
 ...
 */
export async function ensureCommandsDirectory(): Promise<boolean> {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
 ...
 */
export async function createSampleCommand(): Promise<boolean> {
 ...
 */
export async function createNewCommandFile(): Promise<boolean> {
